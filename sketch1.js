...
function drawStartScreen() {
  background("#bde0fe"); // 使用淺色背景
  textAlign(CENTER, CENTER);
  textSize(32);
  fill(0);
  text('歡迎來到遊戲！', width / 2, height / 2 - 150);

  // 顯示玩法說明
  textSize(20);
  text(
    '玩家一: 按W鍵-向上跳, D鍵-向右跑, A鍵-向左跑, S鍵-攻擊\n' +
    '玩家二: 按向上鍵-向上跳, 向右鍵-向右跑, 向左鍵-向左跑, 向下鍵-攻擊',
    width / 2, height / 2 - 50
  );
...
const barWidth = 200; // 放大後的血量條寬度
const barHeight = 20; // 放大後的血量條高度
const textSizeScale = 16; // 放大後的文字大小

// 繪製角色1的名字和血量條
textSize(textSizeScale);
fill(0); // 黑色文字
text('玩家一', 70, 10); // 顯示角色1的名字在血量條上方
fill(255, 0, 0); // 背景條顏色
rect(150, 30, barWidth, barHeight); // 背景條

// 血量條從右側減少
fill(0, 255, 0); // 血量條顏色
rect(150 - (barWidth/2 - char1Health), 30, char1Health *2, barHeight);
 // 根據血量繪製（從右邊減少）
fill(0); // 黑色文字
text(`HP: ${char1Health}`, 80, 55); // 顯示角色1的剩餘能量

// 繪製角色2的名字和血量條
textSize(textSizeScale);
fill(0); // 黑色文字
text('玩家二', width - 220, 10); // 顯示角色2的名字在血量條上方
fill(255, 0, 0); // 背景條顏色
rect(width - 220, 30, barWidth, barHeight); // 背景條

// 血量條從右側減少
fill(0, 255, 0); // 血量條顏色
rect(width - 220 - (barWidth/2 - char2Health ), 30, char2Health * 2, barHeight); // 根據血量繪製（從右邊減少）
fill(0); // 黑色文字
text(`HP: ${char2Health}`, width - 220, 55); // 顯示角色2的剩餘能量
